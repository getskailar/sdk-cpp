...
- The SDK version (the `v0.0.x` tag or vcpkg version) and your compiler and
  platform (for example `clang 17`, `macOS 14`).
...
from an explicit `ClientConfig::api_key`. The key is sent only to the configured
...
overridden by `ClientConfig::default_headers`.
